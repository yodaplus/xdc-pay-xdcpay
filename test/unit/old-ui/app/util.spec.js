...
describe('normalizing values', function () {
  ...
  describe('#ifXDC', function () {
    it('checks if this is XDC chain', function () {
      var result1 = ifXDC(30)
      assert(result1)
      var result2 = ifXDC(31)
      assert(result2)
      var result3 = ifXDC(1)
      assert(!result3)
      var result4 = ifXDC()
  ...
  const addrXDCMainnet = '0xB707b030A7887a21Cc595cD139746A8c2ED91615'
  const addrXDCTestnet = '0xB707b030a7887a21Cc595CD139746a8C2ED91615'
  ...
  describe('#toChecksumAddress', function () {
    it('calculates correct checksum', function () {
      ...
      assert.equal(resultMainnet, addrXDCMainnet)
      var resultTestnet = toChecksumAddress('31', addr)
      assert.equal(resultTestnet, addrXDCTestnet)
      var resultNotXDC = toChecksumAddress('1', addr)
      assert.equal(resultNotXDC, addrETHMainnet)
    })
  })

  describe('#isValidChecksumAddress', function () {
    it('checks if is valid checksum address', function () {
      var resultMainnet = isValidChecksumAddress('30', addrXDCMainnet)
      assert(resultMainnet)
      var resultTestnet = isValidChecksumAddress('31', addrXDCTestnet)
      assert(resultTestnet)
      var resultNotXDC = isValidChecksumAddress('1', addrETHMainnet)
      assert(resultNotXDC)
    })
  })
})
