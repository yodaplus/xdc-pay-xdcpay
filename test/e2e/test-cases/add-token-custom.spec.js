...
const addCustomToken = async (f, account1, account2) => {
  ...
  describe('Check support of token per network basis ', async () => {
    ...
    describe('Token should be displayed only for network, where it was added ', async () => {
      ...
      it('token should not be displayed in XinFin Network', async () => {
      ...
      it('token should not be displayed in XDC network', async () => {
        await f.setProvider(NETWORKS.XDC)
        assert.equal(await f.assertTokensNotDisplayed(), true, 'tokens are displayed')
      })

      it('token should not be displayed in XDC testnet', async () => {
    ...
    describe.skip('Custom tokens validation ', async () => {
      ...
      it('can not add inexistent token to XinFin Network', async () => {
        await f.setProvider(NETWORKS.POA)
        console.log(tokenAddress)
        assert(await f.isDisabledAddInexistentToken(tokenAddress), true, 'can add inexistent token in XinFin Network')
      })
      ...
      it('can not add inexistent token to XDC mainnet', async () => {
        await f.setProvider(NETWORKS.XDC)
        assert(await f.isDisabledAddInexistentToken(tokenAddress), true, 'can add inexistent token in XDC mainnet')
      })

      it('can not add inexistent token to XDC testnet', async () => {
        await f.setProvider(NETWORKS.XDC_TESTNET)
        assert(await f.isDisabledAddInexistentToken(tokenAddress), true, 'can add inexistent token in XDC testnet')
      })
  ...
  describe('Remove token, provider is localhost', function () {
    ...
    it('check if token was removed from XinFin Network', async () => {
      await f.setProvider(NETWORKS.POA)
      assert.equal(await f.assertTokensNotDisplayed(), true, 'tokens are displayed')
    })
